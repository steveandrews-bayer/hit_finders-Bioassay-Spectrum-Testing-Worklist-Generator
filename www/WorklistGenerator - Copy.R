...
#Global Variables
wellNum <- 6 #Used to track location in plate. Will be updated
wellLetter <- "A" #Used to track location in plate. Will be updated
...
add_protein_to_plate <-function(proteinNumber, dilutionNumber, proteinName)
{
  ...
  well$sourceWell <- switch(proteinNumber,
                            "A1", 
                            "A2", 
                            "B1", 
                            "B2", 
...
add_volumes <- function(plate, startingConc, targetVol)
{

  dilNum <- 1
  for(i in 1:length(plate$wells)) 
  {
    dilFactor <- startingConc/targetPPMs[[dilNum]]
...
create_worklists <- function(numProteins, numNormalPlates, numHemipPlates, numLygusPlates, proteinNames)
{
  ...
  allPlates <- list()
  worklist <- list()
  plate <- populate_plate_with_proteins(numProteins, proteinNames)
  startingPlates <- vector(mode = 'list', length = length(numPlates))
  listNames <- rep('wells', length(numPlates))

    for(singlePlate in 1:length(startingPlates))
  {
    startingPlates[singlePlate] <- add_volumes(plate, startingConcs[[singlePlate]], targetVolumes[[singlePlate]])
  }
